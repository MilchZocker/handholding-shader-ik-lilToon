...
// Custom variables - Handholding Shader IK (Arm Hide)
...
// Add vertex shader input - Required for arm hiding based on vertex colors
#define LIL_REQUIRE_APP_COLOR
//#define LIL_REQUIRE_APP_NORMAL
//#define LIL_REQUIRE_APP_TANGENT
//#define LIL_REQUIRE_APP_VERTEXID

// Add vertex shader output - Add custom v2f member for vertex color
#define LIL_CUSTOM_V2F_MEMBER(id0,id1,id2,id3,id4,id5,id6,id7) \
    float4 vertexColor : TEXCOORD##id0;

// Add vertex copy - Copy vertex color to custom v2f member
...
// Inserting a process into pixel shader - Arm Hide Logic
// Discard pixels based on vertex colors at the start of pixel shader
#define BEFORE_UNPACK_V2F \
    if (_EnableFakeArm > 0.5) { \
        if (input.vertexColor.g == 0 && input.vertexColor.b == 0) clip(-1); \
    } \
    if (_EnableFakeLeftArm > 0.5) { \
        if (input.vertexColor.r == 0 && input.vertexColor.g == 0) clip(-1); \
    }
