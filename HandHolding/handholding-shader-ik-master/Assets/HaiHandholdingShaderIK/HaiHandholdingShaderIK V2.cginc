...
float4 maybeFindMatchingLightAsLocalPosition(
    float targetLightIntensity, // intensity to EXCLUDE (ignore lights matching this value)
    float intensityMin,         // minimum accepted light intensity
    float intensityMax,         // maximum accepted light intensity
    float maxDistance)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        // Check if this is a black light (RGB all zero, non-zero position)
        bool isBlackLight = lightColor.r == 0 && lightColor.g == 0 && lightColor.b == 0
            && (unity_4LightPosX0[i] != 0 || unity_4LightPosY0[i] != 0 || unity_4LightPosZ0[i] != 0);
        
        // Check if intensity is within acceptable range
        bool inRange = lightColor.a >= intensityMin && lightColor.a <= intensityMax;
        
        // Check if intensity does NOT match the target (allow small tolerance)
        bool notTarget = abs(lightColor.a - targetLightIntensity) >= 0.0001;
        
        if (isBlackLight && inRange && notTarget)
        {
...
{
    return transformArm(vertex, vertexColor, targetLightIntensity, 0.0001, 0.9999, orElseDefaultLocalPosition, upperarmLength, forearmLength, extraGrabLength, extraGrabLength, extraGrabLength, 0.95, isLeftArm);
}
